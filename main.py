...
def checkLeftRight(img, results, draw=False):
    ...
    if draw:
        ...
        cv2.line(output_image, (width//2, 0), (width//2, height), (255, 255, 255), 2)#white
    ...
    if landmarks:
        if game_started:
            ...
            cv2.putText(img, 'JOIN BOTH HANDS TO START THE GAME.', (5, h - 10), cv2.FONT_HERSHEY_PLAIN,
                        2, (0, 255, 0), 3)#space will be pressed
            
        if checkHandsJoined(img, results)[1] == 'Hands Joined':

            counter += 1
            #this is to count for atleast 2-3 sec  for hands joining
